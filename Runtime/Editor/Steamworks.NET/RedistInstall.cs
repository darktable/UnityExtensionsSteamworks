...
using UnityEditor;
using System.IO;
using UnityExtensions.Steamworks.Editor;
...
public class RedistInstall {
	static RedistInstall() {
		CopyFile($"{Utilities.RuntimeDirectory}/Steamworks.NET/redist", "steam_appid.txt", false);

		// We only need to copy the dll into the project root on <= Unity 5.0
#if UNITY_EDITOR_WIN && (UNITY_4_7 || UNITY_5_0)
	#if UNITY_EDITOR_64
		CopyFile($"{Utilities.RuntimeDirectory}/x86_64", "steam_api64.dll", true);
	#else
		CopyFile($"{Utilities.RuntimeDirectory}/x86", "steam_api.dll", true);
	#endif
#endif
